...
!define PROGRESS_MS 5000      ; 第三页进度条动画总时长（毫秒），按真实墙钟平滑增长
...
Function fn_ProgressPage
  ...
  ${Do}
    ${If} $TickCount < 100
      IntOp $TickCount $TickCount + 1
      StrCpy $ProgressPct $TickCount
      ${If} $ProgressPct < 12
        ${NSD_SetText} $hProgressStatus "正在准备安装..."
      ${ElseIf} $ProgressPct < 70
        ${NSD_SetText} $hProgressStatus "正在解压资源文件... $ProgressPct%"
      ${ElseIf} $ProgressPct < 95
        ${NSD_SetText} $hProgressStatus "正在写入配置文件... $ProgressPct%"
      ${Else}
