...
Function fn_InstallTick
  ; 杀掉当前定时器，避免 Sleep 期间消息泵导致重入叠加（本机定时器间隔近似 0）
  ${NSD_KillTimer} fn_InstallTick
  IntOp $Stage $Stage + 1

  ${If} $Stage == 1
    ${NSD_SetText} $hProgressStatus "正在准备安装..."
    !insertmacro SetProgressWidth 0
  ${ElseIf} $Stage == 2
    ${NSD_SetText} $hProgressStatus "正在解压资源文件... 10%"
    !insertmacro SetProgressWidth 10
  ${ElseIf} $Stage == 3
    ${NSD_SetText} $hProgressStatus "正在解压资源文件... 25%"
    !insertmacro SetProgressWidth 25
  ${ElseIf} $Stage == 4
    ${NSD_SetText} $hProgressStatus "正在解压资源文件... 40%"
    !insertmacro SetProgressWidth 40
  ${ElseIf} $Stage == 5
    ${NSD_SetText} $hProgressStatus "正在解压资源文件... 55%"
    !insertmacro SetProgressWidth 55
  ${ElseIf} $Stage == 6
    ${NSD_SetText} $hProgressStatus "正在写入配置文件... 70%"
    !insertmacro SetProgressWidth 70
  ${ElseIf} $Stage == 7
    ${NSD_SetText} $hProgressStatus "正在写入配置文件... 85%"
    !insertmacro SetProgressWidth 85
    SetOutPath "$INSTDIR"
    FileOpen $R0 "$INSTDIR\installed.txt" w
    FileWrite $R0 "ok"
    FileClose $R0
  ${ElseIf} $Stage == 8
    ${NSD_SetText} $hProgressStatus "正在写入卸载程序... 95%"
    !insertmacro SetProgressWidth 95
