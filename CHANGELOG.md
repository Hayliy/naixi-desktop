...
- **修订号**（`1.0.x`）：向后兼容的缺陷修复与小型改进。
- **次版本号**（`1.x.0`）：向后兼容的新功能。
- **主版本号**（`x.0.0`）：破坏性变更（含用户数据结构调整）——此类变更必须同时提供数据库 / 配置迁移，保证升级不丢数据。
...
> 1.0.1 发布后由用户实测反馈牵出的修复：安装器进度跑完不自动翻页（每次都要手动点一下），
> 以及发布门面自身的两处问题（Release 正文格式漂移、英文 README 长期未同步）。

### 修复（P1 · 安装器进度跑完不自动翻页）
- **现象**：装到第三步（进度页）后进度条已 100%、文案已显示「安装完成。」，页面却停住不动，
  必须手动点底部「完成」才翻页。安装本身只需几分钟，用户看到的是"卡住了"。
- **根因**：进度页计时器在完成时**只被 Kill、不再 Create** ——
  `fn_InstallTick` 只在 `$InstallDone != 1` 时重建计时器，完成后页面自然静止。
- **修复**：新增 `!macro AutoAdvanceNext`，完成后继续跑 8 个 tick（约 1 秒，留时间让用户看到 100% 与
  最终文案）再触发一次「下一步」；安装与卸载（`un.UninstallTick`）同规则。
  安装侧仅在资源完整（`$ResBroken == 0`）时自动翻页 —— 有完整性告警时必须停在原页让用户看到并确认。
  宏内用 `PostMessage` 而非 `SendMessage`：本宏在计时器回调内执行，同步翻页会在回调返回前销毁
  页面对话框，而调用栈深处仍有 nsDialogs 计时器代码未返回，容易踩到悬空状态。
- **连带修复**：此前若安装器被强制关闭（自动化脚本就是强关窗口的），「完成」页从未显示 ⇒
  `fn_Done` 从未执行 ⇒ **桌面快捷方式与"立即运行"从未生效**。自动翻页后该页正常出现。
- **验证**（按用户要求不再重跑安装，改为可复现取证）：从 `installer.nsi` 按边界提取**真实宏与两个
  tick 函数**（不复制粘贴，避免验的不是同一份）用 makensis 编译通过；再用同一份真实宏搭 3 秒独立
  demo，以"页面 leave 回调写出标记文件"为判据 —— 实测**零点击**、2.91 秒后标记出现、退出码 0。

### 修复（P2 · 发布正文格式漂移）
- v1.0.1 的 Release 正文违反仓库自有的 `.github/RELEASE_TEMPLATE.md`：首行是 H1（与 Release 标题
  重复）、章节写成 `## 本版修掉的问题`、条目降到 `###`，在 Release 列表页一眼可见。
- 已按模板重写 v1.0.1 远端正文（H1=0、首行引用块、五段齐全、条目均为 `#### N. 标题（P1）`）。
- 新增 `scripts/check-release-body.mjs` 作为**发布前机器门禁**（禁止 H1 / 首行须引用块 / 五段齐全且
  顺序固定 / 变更内容须分类 / 条目须 `#### N. 标题（P1）` / 围栏成对 / 标题禁 emoji / 下载段须写实际
  资产名），带 `--self-test`；`npm run check:release-body` 可跑，发布脚本推送前调用它，不通过即中止。

### 修复（P2 · sync-version 漏同步英文 README）
- 该脚本原先只处理 `README.md` ⇒ `README_EN.md` 的徽章与下载名长期停在旧版本
  （1.0.1 发布时英文徽章还写着 `Release-v1.0.0`）。
- 已重构为 `syncReadme(file)` 中英共用，各自补语言特有句式（「The X.Y.Z installer uses」/
  「The honest boundary of X.Y.Z:」）。发布后必查两份。

## [1.0.1] - 2026-09-22

> 1.0.0 发布后，由用户报障「虚拟机里点桌宠没反应」牵出的一串修复（含两处会冻结整个后端的 P0/P1 性能缺陷、一处缺单实例保护），以及为「不再靠人肉点界面」新增的两层全量自测（前端全页 + 后端全接口）。均已真机验证。
...
### 修复（P1 · 缺单实例保护：双击两次图标会起两套完整实例并互相踩后端）
- **现象**：真机清理前的快照共 9 个相关进程 —— `naixi-desktop.exe` **3** 个、后端 `pythonw sidecar` 2 个、SearXNG 2 个、桌宠 1 个。用户侧表现为一串难以复现的怪现象：后端 PID 莫名漂移、`pet-switch` 连续切换返回 `ok=false`、界面「点了没反应」。
- **根因**（三处叠加）：
  1. 完全没有单实例保护（`Cargo.toml` 只依赖 `tauri-plugin-shell`，`lib.rs` 只注册了它，前端也未引用 single-instance）——双击两次图标就是**两套完整实例**。
  2. `spawn_backend` 的端口短路在多实例下**放大**故障：第二个实例见 9845 被占就 `return Ok(())` 跳过启动后端，于是两个前端连到同一个后端；而第一个实例退出时会 `kill_backend` 杀掉它，第二个实例瞬间失去后端。这是「PID 漂移」与「连续切换失败」的成因。
  3. `kill_backend` 的兜底分支对「监听 9845 的任意进程」直接 `taskkill /F /T` —— 该端口若被用户自己的程序占用，退出时会**误杀**。
- **修复**：① 引入 `tauri-plugin-single-instance` 并在 Builder 最前注册，第二个实例被拦截时唤出已有窗口（用户能看出「它已经在运行」）；② 新增 `is_own_backend(pid)`（进程名属 python 系 **且** 命令行含 `naixi`），`spawn_backend` 在端口被占时只清理自家残留后端，遇到别的程序则返回明确错误、不硬抢；③ 退出兜底清理同样先过该判据，认不出就不动手。
- **真机验证**：启动前先清到 0 个进程，启动后 `main=1 / backend=1`；再启动第二个实例，进程数保持不变（守卫生效）。

### 修复（P1 · 自动化调度整轮被跳过）
- **根因**：调度器的 RRULE 解析写成 `{k: v for kv in rrule.split(";") for k, v in [kv.split("=")] if "=" in kv}` —— **先解包、后过滤**（`if` 在推导式末尾生效），因此 RRULE 里只要出现一个不含 `=` 的段（例如结尾多一个 `;` 产生的空段），就抛 `not enough values to unpack`，被外层捕获后跳过整轮调度（睡 30 秒再来），表现为**该轮所有自动化都不执行**。
...
>
> 稳定期的核心承诺：**用户数据向后兼容**。本地 SQLite 数据库与配置文件的任何破坏性变更都必须登记迁移函数并向前兼容。

### 新增（P1 · 应用内诊断）
- **诊断端点 `GET /api/diagnostics`**（`desktop_core/diagnostics.py`）与配置 schema 版本化模块（`config_schema.py`）：一次返回后端存活、Python 版本、配置 / 数据库 schema 版本、平台连接器数量、已配置供应商与平台、直播引擎状态、健康评分与降级项清单。
- **前端入口**：TopBar「系统」菜单新增「运行诊断」，以可读弹窗呈现上述结果。
- 价值：此前「功能静默失效」（依赖缺失、降级路径）只能靠读日志发现；现在用户与支持者有了统一的自检入口。
- 配套新增 `docs/DEGRADATION_AUDIT.md`：逐条登记代码里 `try/except` 静默降级点、触发条件与用户可感知后果。

### 修复（P1 · 用户数据安全）
#### 卸载不再删除用户数据
- **修复前**：`installer.nsi` 卸载段无条件执行 `RMDir /r "$INSTDIR\resources"`，会连带删除 `resources\data`（本地 SQLite 数据库：对话、知识库、长期记忆、配额）与 `resources\workspace`。用户卸载或重装即丢失全部资产，且没有任何提示。
- **修复后**：卸载只删除纯程序资源（`desktop_core` / `python-embed` / `searxng` / `plugins`）；`data` 与 `workspace` 仅在用户在卸载确认页勾选「同时删除我的个人配置与数据」时才删除（默认不勾），完成页告知数据保留位置。
- **真机验证**：卸载后数据库、工作区与预置标记文件全部保留，同时 `desktop_core` 被正确删除。

### 修复（P1 · 产物与发布契约）
#### 1. 停止发布装不出东西的 MSI
- `bundle.targets` 由 `"all"` 收为 `["nsis"]`：WiX(MSI) 模板未同步 7z 聚合资源方案，装完没有 `desktop_core` / `python-embed`，后端起不来——此前每版都把它与 NSIS 并排挂在 Release 上。
- 发布卡口新增断言：产物目录不得存在 `.msi`。

#### 2. 发布版关闭开发者工具
- `open_devtools` 命令在 release 构建中直接拒绝；窗口 `devtools` 由构建流程注入为 false。此前 release 版仍可从 UI 打开 DevTools。

#### 3. 发布资产与版本契约
- 新增 `scripts/release_guard.py`：校验工作区干净、HEAD 已推远端、五处版本号一致、安装包存在且已签名、无 `.msi`、`SHA256SUMS.txt` 覆盖当前安装包且含 [主程序] 组、签名公钥就位；并产出 `build-manifest.json`（提交 SHA / 依赖清单哈希 / 产物 SHA256 / 签名主体与指纹 / 发布资产清单）。
- **修复发布资产缺失**：此前 Release 只上传安装包本身（v0.2.10 实测 1 个资产），而 README 指引用户下载 `SHA256SUMS.txt` 校验、下载公钥核验签名 —— 文档与产物脱节，用户无法执行。现由 `scripts/build-release.ps1` 一并产出并列出待上传资产。
- **修复清单文件名不一致**：脚本产物是小写 `sha256sums.txt`、Release 资产是大写 `SHA256SUMS.txt`，Linux / macOS 用户按文档名下载会 404。现全局统一为 `SHA256SUMS.txt`。

### 新增（P1 · 代码签名）
- `scripts/gen-selfsign-cert.ps1`：生成 Code Signing EKU 的自签名证书并导出公钥 `.cer`。
- `scripts/build-release.ps1`：注入证书指纹构建 → 安装包签名 + RFC3161 时间戳 → 自检 → 产出发布资产 → 跑发布卡口。
- `scripts/sign-app-binary.ps1`：在打包前给主程序签名（`beforeBundleCommand` 钩子）。
- `docs/CODE_SIGNING.md`：写清自签名的边界（**不消除 SmartScreen「未知发布者」**，给的是完整性与签名主体可核验）、核验方法与换受信任 CA 证书的流程（2023-06 起 CA/B 强制私钥入 HSM，不再能用 PFX）。
- 换正式证书只需替换指纹，构建与卡口流程不变。

### 新增（P1 · 合规）
- `scripts/gen_third_party_licenses.py`：从安装包真实内容生成完整第三方清单（python 118 + 前端 244 组件，copyleft 12 个，未识别 0 个），并复制 copyleft 许可证原文。
- 本轮新挖出的分发义务：`espeakng-loader` 内嵌 `espeak-ng.dll` 与语音数据（GPL-3.0）、`pywin32` 附带 `adodbapi`（LGPL-2.1）、`pillow` 的 LICENSE 内含 GPL 段落、numpy / opencv 附带 copyleft 子组件。`NOTICE` 由 4 个组件改为完整清单指针 + 分发义务与源码获取途径。
- `docs/PRIVACY.md`：按代码中真实出站主机逐条列数据流向、触发条件与关闭方式，写明不采集遥测与卸载数据保留策略。

### 新增（P2 · 质量门禁）
- 前端 `build` 改为 `tsc --noEmit && vite build`（`strict: true` 此前从不执行）；新增 `typecheck` 脚本。
- 新增 `tests/unit_test.py`（零框架自包含）：路径不变量、配置迁移不变量、密钥掩码语义、DB schema 迁移幂等与失败不推进、诊断契约。实跑全量 19/19、轻量 8/8。
- CI 增加类型检查与两道单元测试步骤。

### 其他
- **仓库卫生**：移出误入库的 Windows 快捷方式与 52 MB 实验素材（本地文件保留）；发布哈希清单不再入库（仓库内留旧清单会与最新 Release 不一致，用户照它校验必然失败）。
- **README 修正**：更正「19 平台接入」等与实际不符的表述（实际 12 个连接器，且微信 / 公众号 / LINE 并不存在）、更新项目规模统计、新增「系统要求」章节、重写「升级与卸载」以匹配新的数据保留行为、补记 Godot 导出产物需自备。

### 验证
- **构建链路**：`tsc --noEmit` + `vite build` + stage-core（55 个 .py）+ `build.rs` 依赖守卫（导入探针 OK 29 / FAIL 0）+ 7z 聚合（21 卷）+ Rust 编译，全部通过。
- **单元测试**：全量 19/19 PASS；轻量（仅 stdlib）8/8 PASS。
- **真机（VMware 全新安装）**：`/api/diagnostics` 实取 —— `backend=running`、`python=3.13.12`、`config_schema_version=1`、`db_schema_version=1`、`platform_catalog_count=12`、`health_score=80`、`degradations` 为空；安装目录 1.26 GB / 21,482 文件。
- **真机（VMware 卸载）**：用户数据库、工作区与预置标记文件全部保留，`desktop_core` 被正确删除。
- **签名**：安装包与安装后的主程序均带 Authenticode 签名（自签名证书 `CN=Naixi Desktop (Self-Signed)`）；指纹与产物哈希见 `build-manifest.json`。

### 升级说明
- **覆盖安装**：直接运行新安装包，无需先卸载；程序文件被替换，`data/` 与 `workspace/` 不受影响。
- **未签名提示**：自签名证书的根不受 Windows 信任，SmartScreen 仍会提示「未知发布者」，属预期行为；请按 README「安全与完整性」完成哈希校验。
- **发布范围**：仅 NSIS（x64）。MSI 不发布（见上文「产物与发布契约」）。
- **从 0.2.x 升级**：卸载行为已改变 —— 本版及以后卸载默认保留用户数据。

## [0.2.10] - 2026-09-21

> 由真机全功能测试产出，全部经 VM 全新安装复验；并补齐此前被标记为「占位」的快捷键系统与对话页默认头像。（本条目为补记：该版本已发布但当时未写入本文件。）

### 新增
- **快捷键系统真正实现（P1）**：以 `src/lib/shortcuts.ts` 为单一真相源，导出 6 个真实动作（发送消息 / 换行 / 上一条消息 / 新对话 / 全局动作等），支持改键并经 CustomEvent 即时生效。修复前对话框仅有占位文本、按键无响应；修复后 ChatInput 动态提示「Enter 换行 · Ctrl+Enter 发送 · ↑ 上一条」，4 个组件真消费。
- **内置 50 张二次元猫娘默认头像（P2）**：对话页默认头像改为随安装包分发的本地图，离线可用。修复前新装机走外网 DiceBear 占位 SVG；修复后 `resolveAvatarUrl` 默认兜底走 `avatars/avatar-N.png`（public / dist 各 50 张，随 NSIS 分发）。
- **发布规范模板（QA）**：新增 `.github/RELEASE_TEMPLATE.md`，统一标题与五段正文（含可量化验证）。

### 修复
- **版本号显示修复（P1）**：修复前装完后前端侧边栏显示 `v0.0.0-dev`、TopBar 显示 0.2.1，两处不一致；修复后 `version.json` 随安装包进入 resources，前端两处一致显示 0.2.10。

### 其他
- **资源聚合打包（P1）**：安装包改为 7z 聚合资源，根治逐文件解压导致的安装第三页冻结 2–3 分钟。

### 验证
- 版本号链路（VM 真机全新安装）：`version.json` / 注册表 / `/api/status` / `/api/desktop_status` 四处一致 —— 4/4 PASS。
- 快捷键 6/6 动作真实绑定、4/4 组件真消费；猫娘头像 public=50、dist=50 —— 100% 命中。
- 全新装机各表 count=0、数据库 0.22 MB、后端与 SearXNG 服务正常；NSIS 安装后后端 9845 起服，三端点取证成功。

### 升级说明
- 覆盖安装：直接运行新安装包，旧资源目录由静默清道夫清理，无需手动卸载。
- 发布范围：仅 NSIS（x64）；MSI 因 7z 聚合资源方案未同步 WiX 模板，本版不发布。

## [0.2.9] - 2026-09-20

> 由 0.2.8 真机全功能深度测试（VMware 全新装机 + 黑盒 E2E）产出的 4 项修复组成，全部经真机复验。（本条目为补记：该版本已发布但当时未写入本文件。）

### 修复
- **VRM 模型导入失败（P1）**：修复前导入任何大于 1 MB 的 `.vrm` 都会失败（服务端默认 1 MB 上传限制）；修复后上限放宽到 2 GB，353 MB 模型实测 3.9 秒导入成功。
- **指定模型启动桌宠不生效（P1）**：修复前后端忽略传入的模型路径，永远显示「还没有模型」占位卡；修复后显式指定模型路径时优先按该模型启动（2D / 3D 按扩展名自动分流）。
- **导入模型后启动桌宠仍显示「还没有模型」（P2）**：修复前导入 VRM 后直接点「启动桌宠」（默认 2D 偏好）仍提示没有模型；修复后当前偏好模式下没有模型时自动回退到另一种渲染模式（2D ↔ 3D）。
- **占位卡「粘鼠标」（P2，用户反馈）**：根因是模态导入窗打断了「按下 → 抬起」事件对，拖拽状态残留；修复后打开导入窗前显式复位拖拽状态，并加鼠标移动自愈检测。

### 验证
- 发版三关：commit + push、真机验证、发布。
- 代码一致性：guest 实机运行的 `desktop_core` 文件 md5 与 0.2.9 源码逐一比对一致。
- 直播模块 21/21 断言 PASS；VMC OSC 收到首包 1.67s，口型包到达 UDP 39539。
- 真实 LLM 对话（真机 key）：chat_stream 3.5s 返回真实回复；直播弹幕链路 1.61s + TTS 6.07s 出声。
- 全功能测试 7/7 模块 PASS（知识库 / 聊天降级 / 设置持久化 / 直播 / VRM-Live2D / 自动化 / 真实 LLM）。

### 升级说明
- 支持覆盖安装：安装器先清理旧版程序目录（数据 / 模型 / 日志保留）。
- 安装过程若杀软实时防护开启，解压阶段可能稍慢，属正常现象。

## [0.2.8] - 2026-09-20

> 本版本源自用户反馈「仪表盘系统资源数据明显不对」，由此倒查发现**多处指标在真机上静默失真**（页面不报错、控制台干净，常规功能测试抓不到）。核心是把系统指标收拢到单一真相源 `desktop_core/sys_metrics.py`（ctypes 直读 Windows API），不再依赖 psutil 7.x 的已知缺陷与 `dirname(__file__)` 路径假设。

### 修复（P1 · 仪表盘/运维页指标失真）
- **CPU 占用恒 0**：`psutil.cpu_percent()` 在 7.x 中**同一进程第二次调用必返回 0.0**（首调用建立基线、间隔内无采样），导致仪表盘 CPU 永远 0、健康分虚高到 100、自检 `cpu_pct>95` 分支永不触发、巡检 inspect CPU 恒 0。现新增 `sys_metrics.py`，用 `GetSystemTimes` 双采样求差，**稳定给出 40–70% 区间的真实占用**。
- **数据库大小恒 0MB**：`api_stats` 用 `dirname(__file__)` 拼 `../data/naixi_desktop.db`，dev/打包态下 `__file__` 落在 `resources/desktop_core`，上溯两级并非真实数据目录 → 找不到文件 → 0MB。现统一读 `storage.DB_PATH`（含多级候选回退）。
- **GPU 无卡时显示假数据**：无 NVIDIA 显卡/驱动时 `nvidia-smi` 不可用，前端却渲染「0% / N/A / 0 / 0MB」——看起来像「检测到了但没负载」，实际是假值。现后端返回 `gpu_available` 标志，前端无卡时显式提示「未检测到 NVIDIA 显卡」。
- **磁盘占用随工作目录漂移**：`disk_usage("/")` 在 Windows 上按「当前进程 CWD 所在盘」解析，dev 下 CWD 在 `D:` 时显示的是 D 盘而非系统盘。现强制用 `%SystemDrive%`。
- **数据库大小精度不一致**：运维端点 `round(x,1)`、统计端点 `round(x,2)` 两处口径不同，对拍时差 0.1MB 误报。统一为 `round(x,2)`。

### 修复（P1 · 知识库输入校验）
- **知识库标题/内容无长度上限**：实测可提交 20000 字标题，既不合常理也易撑爆数据库字段。现 `knowledge_add` 增加 `title≤200 / content≤50000 / category≤50` 校验，超限返回 400 并附带当前长度。
- **非法请求泄漏内部异常**：`knowledge_add` 解析到坏 JSON 时把堆栈原样抛给前端（含文件路径/代码行），既泄露实现又难看。现 catch-all 返回友好文案「请求格式有误，请检查后重试」。

### 新增（QA · 防回归）
- **前端字段 vs 后端响应契约审计**（`scripts/audit_frontend_api_contract.py`）：采集每个端点运行时返回的键，逐组件比对「前端读取了但后端从不返回」的字段——正是仪表盘 `gpu_*` 长期失真的同类信号（读了但后端不返回，静默显示 0/N/A）。需逐条确认。

### 验证
- 真机（VMware）`verify_fix.py`：CPU 连续采样 61.8 / 55.9 / 54.1%、内存 62.4%、磁盘 65.9%（系统盘）、db 28.6MB；ops 页系统资源块与后端进程块均非 0；
- `deeptest.py` 黑盒（并发 12×3、连续采样、边界、一致性）：db 精度、知识库长度上限、坏 JSON 友好响应三项全部通过；
- `py_compile` 与前端 `tsc --noEmit` 通过。
